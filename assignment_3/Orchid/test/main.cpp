...
int main(int argc, char *argv[])
{
	...
	(
		Vector3d(278.0f, 273.0f, -800.0f),	// position
		Vector3d(278.0f, 273.0f, 0.0f),		// target
		Vector3d(0, -1, 0),					// up
		200, 200,		//camera width and height
		0.1,
		40
	);     // Create camera
	...
	(
		...
		(
			...
			(
				...
				Material(DIFF, { 1.0f, 1.0f, 1.0f })		//bottom
			)
			)
	);
	scene.add
	(
		...
		(
			...
			(
				...
				Material(EMIT, { 1.0f, 1.0f, 1.0f }, { 25.0f, 25.0f, 25.0f })	//light
			)
			)
	);
	scene.add
	(
		...
		(
			...
			(
				...
				Material(DIFF, { 1.0f, 1.0f, 1.0f })	//ceiling, white
			)
			)
	);
	scene.add
	(
		...
		(
			...
			(
				...
				Material(DIFF, { 1.0f, 1.0f, 1.0f })	// back wall, white
			)
			)
	);
	scene.add
	(
		...
		(
			new Quad(
				...
				Material(DIFF, { 0.0f, 1.0f, 0.0f })// Green, left wall
			)
			)
	);
	...
	(
		...
		(
			...
			(
				...
				Material(DIFF, { 1.0f, 0.0f, 0.0f })	//Red, right wall
			)
			)
	);
	...
	//scene.add( dynamic_cast<Object*>(new Mesh(Vector3d(), "objFile/teddy.obj", Material(DIFF, Vector3d(0.9, 0.9, 0.9)))) );
